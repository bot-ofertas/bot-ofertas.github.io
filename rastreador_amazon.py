...
async def rodar_uma_vez() -> None:
    ...
    # Registrado na tabela `execucoes` — é o que core.database.
    # execucao_em_andamento() consulta pro desligamento noturno saber que
    # o Amazon está no meio de uma rodada (busca + posts Telegram/WhatsApp)
    # e esperar. Faltava aqui até 2026-08-03 — Amazon nunca marcava
    # "ocupado", então um desligamento podia interromper um envio de
    # WhatsApp em andamento sem que verificar_ocioso.py soubesse esperar.
    ...
    try:
        ...
        # confirmado ao vivo em rastreador.py/campanha_ferramentas.py em
        # 2026-08-03.
    ...
    if args.loop or args.random:
        ...
        while True:
            try:
                ...
                from core.error_logger import log_erro  # noqa: PLC0415
                log_erro("amazon.rodada_falhou", e, {})
                log(f"⚠️  Rodada Amazon falhou inesperadamente: {e}")
